...
train_tensor = torch.Tensor(train_input)
...
class LSTMModel(nn.Module):
    ...
    def forward(self, x):
        h, c = self.lstm(x)
        output_fc = self.fc(h[:, -1, :])
        output=torch.softmax(output_fc, dim=1)
        return output

# 创建模型、损失函数和优化器，并放到设备上
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model = LSTMModel(50, 50, 2,dropout_rate=0.1).to(device)
...
for epoch in range(epoch_tot):
    ...
    plt.ylim(0.5, 0.65)  # 设置y轴范围
    plt.yticks(np.arange(0.5,0.65, 0.05))  # 设置y轴刻度
    plt.pause(0.01)

    if epoch_loss < min_loss:
        min_loss = epoch_loss
        torch.save(model.state_dict(), 'model.pt')  # 只保存模型参数
